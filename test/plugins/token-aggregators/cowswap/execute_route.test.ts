...
import { sign_order, submit_order } from '@benchmarking-bridge-aggregators/plugins/token-aggregators/cowswap/execute_route';
import { COWReturn, COWOrderRequest } from '@benchmarking-bridge-aggregators/plugins/token-aggregators/cowswap/types';
...
describe.skip('should submit an order for WETH -> USDC', () => {
    it('should create a WETH SELL order on WETH to USDC on GOERLI', async () => {
        ...
        const cowReturn: COWReturn = await build_route(sourceChain, destChain, fromToken, toToken, amount, operation);
        const orderRequest: COWOrderRequest = cowReturn.orderReq;
        const order: Order = cowReturn.order;
